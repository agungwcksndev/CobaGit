...
class Kbagian extends CI_Controller{
	...
	function index(){
		$tahun = date("Y");
		$data['main_view'] = 'tatausaha/kepala_bagian/dashboard_view';
		$data['surattugas'] = $this->m_kbagian->get_bulan_surat_tahun($tahun);
		$data['pribadi'] = $this->m_kbagian->get_bulan_surat_pribadi_tahun($tahun);
		$data['pegawai'] = $this->m_kbagian->get_count_pegawai();
		$data['surat'] = $this->m_kbagian->get_count_surat();
		$data['periode'] = $this->m_kbagian->get_periode($tahun);
		$data['bgn'] = $this->m_kbagian->get_jum_surat_bagian($tahun);
		$data['surat_terendah'] = $this->m_kbagian->get_jum_surat_bagian_terendah($tahun);
		$data['jumsuratpribadipertahun'] = $this->m_kbagian->get_jum_surat_pribadi_pertahun($tahun);
	...
	function tampil_surat(){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_tampil';
	...
	function detail_surat($Nomor_Surat){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_detail_surat';
		$data2['main_view'] = 'tatausaha/kepala_bagian/v_buat_detail_surat';
	...
	function detailsurat(){
		...
		$url=$values[$length-4].'/'.$values[$length-3].'/'.$values[$length-2].'/'.$values[$length-1]; 
		$data['surattugas'] = $this->m_kbagian->get_no_surat($url);   
		$data['main_view'] = 'tatausaha/kepala_bagian/v_buat_detail_surat';
		$this->load->view('template',$data);
	}

		// lihat list TU	
    function PegawaiTU(){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_pegawai';
	...
	function cari_surat(){
		...
		$this->load->view('tatausaha/kepala_bagian/v_tampil', $data);
	}	
	...
	function Surat($NIP){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_surat';
		$data2['main_view'] = 'tatausaha/kepala_bagian/v_pegawai';
		$page = $_SERVER['PHP_SELF'];
		$values=explode("/",$page); 
		$length=sizeof($values); 
		$url=$values[$length-1]; 
		$cek = $this->m_kbagian->cek_ketersediaan_surat($url)->num_rows();
		if($cek > 0){
			$data['detail_st'] = $this->m_kbagian->SuratTugas($url);
			$this->load->view('template',$data);
		}else{
	...
	function PencarianSurat(){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_surattugas';
		$bln1 = $this->input->get('bln1');
		$bln2 = $this->input->get('bln2');
		$tahun = $this->input->get('tahun');
		$nip = $this->input->get('NIP');
		$data['detail_st'] = $this->m_kbagian->SuratTugs($bln1,$bln2,$tahun,$nip);
		$data['surat'] = $this->m_kbagian->surat_rekap_perbulan($nip);
		$data['detail'] = $this->m_kbagian->rekap_surat_perbulan($nip);
		$this->load->view('template',$data);
	}

	function excelsuratpegawaiperbulan($nip,$bulan){
		$data['detail_st'] = $this->m_kbagian->surat_perbulan_detail_form($nip,$bulan);
		$data['surattugas'] = $this->m_kbagian->surat_perbulan_detail($nip,$bulan);
		$data['surattugas'] = $this->m_kbagian->tampil_surat();   
		$this->load->view('admin/export/v_surat_tugas_pegawai_perbulan', $data);
	}

	function rekapsurat($NIP){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_datasurat';
		$data['detail_st'] = $this->m_kbagian->surat_perbulan($NIP);
		$data['surat'] = $this->m_kbagian->surat_rekap_perbulan($NIP);
		$data['detail'] = $this->m_kbagian->rekap_surat_perbulan($NIP);
		$this->load->view('template',$data);
	}
	function detailrekapsuratform($NIP,$bulan){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_rekap_bulan_pegawai';
		$data['detail_st'] = $this->m_kbagian->surat_perbulan_detail_form($NIP,$bulan);
		$data['surattugas'] = $this->m_kbagian->surat_perbulan_detail($NIP,$bulan);
		$this->load->view('template',$data);
	}
	...
	function data_admin(){
		$data['main_view'] = 'tatausaha/kepala_bagian/v_data_admin';
	...
	function rekap(){	  
		...
		$data = array();
		$data['main_view'] ="tatausaha/kepala_bagian/v_rekap";
